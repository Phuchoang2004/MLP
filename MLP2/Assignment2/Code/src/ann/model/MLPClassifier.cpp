...
double_tensor MLPClassifier::predict(
    ...
    for(auto batch: *pLoader){
        ...
        nsamples += X.shape()[0];
        string progress = fmt::format("{:6d}/{:12d}|{:50d}\n",
...
double_tensor MLPClassifier::evaluate(DataLoader<double, double>* pLoader){
    ...
    for(auto batch: *pLoader){
        ...
        ulong_tensor y_true = xt::argmax(t, -1);  // Changed axis to -1
        ulong_tensor y_pred = xt::argmax(Y, -1);  // Changed axis to -1
...
void MLPClassifier::backward(){
    //YOUR CODE IS HERE
    // Initialize the gradient with the loss gradient
    xt::xarray<double> gradient = m_pLossLayer->backward();

    // Propagate the gradient through each layer in reverse order
