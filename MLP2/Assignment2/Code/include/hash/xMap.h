...
V xMap<K,V>::remove(K key,void (*deleteKeyInMap)(K)){
    ...
    for(typename DLinkedList<Entry*>::Iterator it = bucket.begin(); it != bucket.end(); it++){
        if(keyEQ((*it)->key, key)){
            ...
            V value = (*it)->value;
            Entry* entryToRemove = *it;
            
            // If deleteKeyInMap function is provided, call it on the key
            if(deleteKeyInMap != nullptr){
                deleteKeyInMap(entryToRemove->key);
            }
            
            // Remove the entry from the bucket
            bucket.removeItem(entryToRemove);
    ...
    // If key is not found, throw KeyNotFound exception
    stringstream os;
    os << "key (" << key << ") is not found";
    throw KeyNotFound(os.str());
}
...
bool xMap<K,V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V)){
    ...
    // Search for the entry with matching key and value
    for(typename DLinkedList<Entry*>::Iterator it = bucket.begin(); it != bucket.end(); it++){
        if(keyEQ((*it)->key, key) && valueEQ((*it)->value, value)){
            ...
            if(deleteValueInMap != nullptr){
                deleteValueInMap(entryToRemove->value);
            }
...
bool xMap<K,V>::containsKey(K key){
    ...
    DLinkedList<Entry*>& bucket = table[index];
    for(typename DLinkedList<Entry*>::Iterator it = bucket.begin(); it != bucket.end(); it++){
        if(keyEQ((*it)->key, key)) return true;
    }
    return false;
}
...
bool xMap<K,V>::containsValue(V value){
    ...
    for(int i=0;i<capacity;i++){
        ...
        for(typename DLinkedList<Entry*>::Iterator it = bucket.begin(); it != bucket.end(); it++){
            if(valueEQ((*it)->value, value)) return true;
        }
    }
    return false;
}
