...
T Heap<T>::pop(){
    //YOUR CODE IS HERE
    if(count == 0) throw std::runtime_error("Calling to peek with the empty heap.");
...
void Heap<T>::remove(T item, void (*removeItemData)(T)){
    ...
    if(removeItemData != 0) removeItemData(elements[index]);
    elements[index] = elements[count - 1];
    count--;
    
    if(count > 0 && index < count) {
        int parent = (index - 1) / 2;
        if(index > 0 && aLTb(elements[index], elements[parent]))
            reheapUp(index);
        else
            reheapDown(index);
    }
