...
void XArrayList<T>::copyFrom(const XArrayList<T> &list)
{
    ...
    data = new T[capacity];
    memcpy(data, list.data, count * sizeof(T));
}
...
void XArrayList<T>::removeInternalData()
{
    ...
    if(deleteUserData!=nullptr){
        ...
        delete[] data;   // First free the memory.
        data = nullptr;  // Then set the pointer to nullptr to avoid dangling pointer issues.
        count = 0;
        capacity = 10;
    }
}
...
XArrayList<T> &XArrayList<T>::operator=(const XArrayList<T> &list)
{
    // TODO
     if (this != &list) {
        removeInternalData();  // Free existing memory before assigning new data.
        copyFrom(list);
    }
    return *this;
}
...
bool XArrayList<T>::removeItem(T item, void (*removeItemData)(T))
{
    ...
    int index = indexOf(item);
        if (index != -1) {
            T CutRaNgoai = removeAt(index);
              if(removeItemData!=NULL) removeItemData(CutRaNgoai);
              return true;
        }
        return false;
}
...
void XArrayList<T>::clear()
{
    ...
    delete[] data;
    data = new T[initcap];  // Then set the pointer to nullptr to avoid dangling pointer issues.
    count = 0;
    capacity = 10;
}
...
void XArrayList<T>::ensureCapacity(int index)
{
    ...
    if(index>=capacity||count==capacity){
        try{
            int Capybara = capacity*2; // Ensure the new capacity is sufficient
            T* newdata = new T[Capybara];
            memcpy(newdata,data,count*sizeof(T));
            delete[] data; // Proper deletion
            data = newdata;
            capacity = Capybara;
        }
        catch(const bad_alloc& e){
            cerr<<"Index is out of range!"<< e.what()<<endl;
            throw;
        }
    }
